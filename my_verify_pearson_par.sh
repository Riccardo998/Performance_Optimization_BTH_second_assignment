...
do
    for size in 128 256 512 1024
        ...
        ./pearson_par/pearson "pearson_par/data/$size.data" "$temp_dir/${size}_par.data" "${num_threads}"
...
#
        ...
        #        echo "${red}Error: Mismatch in output data for size $size with $num_threads thread(s)${reset}"
...
#
        ...
        #            echo "${yellow}Warning: Minor differences (last four digits) detected for size $size and $num_threads thread(s)${reset}"
        #            warning_issued=1
        #        fi
        #    fi
        #done < "${size}_seq.data" 3< "$temp_dir/${size}_par.data"
#
        #if [ "$difference_found" -eq 0 ]; then
        #    echo "Files match for size $size with $num_threads thread(s)"
        #fi

        if ! cmp -s "${size}_seq.data" "$temp_dir/${size}_par.data"
        then
            echo "${red}Incongruent output data detected with size $size"
            status=1
        fi

        rm "$temp_dir/${size}_par.data"
    done
done
